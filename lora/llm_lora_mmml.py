...
class LLM_Lora(object):
    ...
    def _eval(self,
        ...
        # Check the values
        if (instruction is None) or (input is None) or (options is None):
            print(f"Instruction: {instruction}\nInput: {input}\nOptions: {options}")
            raise ValueError("Instruction, input and options cannot be None")

        # Format the prompt
        prompt = self.prompter.generate_prompt_mmml(
    ...
    def generate_and_tokenize_prompt(self, data_point):
        full_prompt = self.prompter.generate_prompt_mmml(
